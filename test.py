...
class MyTabView(ctk.CTkTabview):
    def __init__(self, master, **kwargs):
        ...
        self.add("Visualization Tools")  

        self.create_sentiment_analyzer_tab()
        self.create_visualization_tools_tab()
    ...
    def create_sentiment_analyzer_tab(self):
        ...
        self.upvotes_slider = ctk.CTkSlider(master=self.sliders_frame, height=200, from_=0, to=10, number_of_steps=10, orientation="vertical")
        self.upvotes_slider.grid(row=1, column=0, padx=(60, 0), pady=(0, 100), sticky="sw")
        self.upvotes_value_label = ctk.CTkLabel(master=self.sliders_frame, text="5", font=("Arial", 13))
        self.upvotes_value_label.grid(row=1, column=0, padx=(2, 0), pady=(0, 70), sticky="swe")
        self.upvotes_slider.configure(command=self.update_upvotes_label)
        ...
        self.total_votes_slider = ctk.CTkSlider(master=self.sliders_frame, height=200, from_=0, to=10, number_of_steps=10, orientation="vertical")
        self.total_votes_slider.grid(row=1, column=1, padx=(64, 0), pady=(0, 100), sticky="sw")
        self.total_votes_value_label = ctk.CTkLabel(master=self.sliders_frame, text="5", font=("Arial", 13))
        self.total_votes_value_label.grid(row=1, column=1, padx=(0, 4), pady=(0, 70), sticky="swe")
        self.total_votes_slider.configure(command=self.update_total_votes_label)
        ...
        self.rating_slider = ctk.CTkSlider(master=self.sliders_frame, height=200, from_=1, to=5, number_of_steps=4, orientation="vertical")
        self.rating_slider.grid(row=1, column=2, padx=(48, 0), pady=(0, 100), sticky="sw")
        self.rating_value_label = ctk.CTkLabel(master=self.sliders_frame, text="3", font=("Arial", 13))
        self.rating_value_label.grid(row=1, column=2, padx=(0, 6), pady=(0, 70), sticky="swe")
    ...
    def create_visualization_tools_tab(self):
        self.label = ctk.CTkLabel(master=self.tab("Visualization Tools"))
